...
def BCCgen_111_perfect():
	...
	m = 15; n = 9; cart_atoms = []; count=0; pos_screw = []; pos_perfect = [];
...
# direct coordinates -- > Cartesian	
	...
	with open("POSCAR_perfect","w") as fdata2:
		...
		if (sys.argv[2] == "c" or sys.argv[2] == "C"):
			...
			for i in range(len(pos_perfect) ):
			#print("{:12.9f} {:12.9f} {:12.9f}".format(cart_atoms[i][0], cart_atoms[i][1], cart_atoms[i][2]))
				fdata2.write("{:12.12f} {:12.12f} {:12.12f} T T T\n".format(pos_perfect[i][0]/lat_par, pos_perfect[i][1]/lat_par, pos_perfect[i][2]/lat_par ))

	return pos_perfect, count, basis, lx, ly, lz

'''##################################################################################
#------------------ Creating screw dislocation for randomise HEA -------------------#
##################################################################################'''	
...
#***************Position of dislocation line at Ci(X, Y, Z) for +/-b*********
	...
	C1X = 0.25*lx + delta  ; C2X = 0.75*lx + delta 
	C1Y = 0.25*ly 				 ; C2Y = 0.25*ly 			  
	C3X = 0.75*lx + delta  ; C4X = 0.25*lx + delta	
	C3Y = 0.75*ly 			   ; C4Y = 0.75*ly 						
	C1Z = C2Z = C3Z = C4Z = lz
...
if __name__ == '__main__':
	...
	elif ((sys.argv[1] == 'bcc' or sys.argv[1] == 'fcc') and (sys.argv[2] == 'c' or sys.argv[2] == 'd') and sys.argv[3] == 'sup'):
